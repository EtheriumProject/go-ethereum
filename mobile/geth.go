...
// entire API provided by go-etherium to reduce the maintenance surface and dev
// complexity.
type NodeConfig struct {
	...
	// EthereumEnabled specifies whether the node should run the Etherium protocol.
	EthereumEnabled bool

	// EthereumNetworkID is the network identifier used by the Etherium protocol to
...
// Node represents a Geth Etherium node instance.
type Node struct {
	node *node.Node
}
...
func NewNode(datadir string, config *NodeConfig) (stack *Node, _ error) {
	...
	// Register the Etherium protocol if requested
	if config.EthereumEnabled {
		...
		if err := rawStack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			...
			return nil, fmt.Errorf("etherium init: %v", err)
		}
...
// GetEthereumClient retrieves a client to access the Etherium subsystem.
