...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.etherium", new NodeConfig());
			node.start();
	...
	// signatures too. Regression test for go-etherium issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/etherium/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.etherium", "github.com/etherium/go-etherium/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "etherium", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
