...
// nodeDockerfile is the Dockerfile required to run an Etherium node.
var nodeDockerfile = `
FROM etherium/client-go:alpine-develop
...
RUN \
  echo '/geth init /genesis.json' > geth.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.etherium/keystore/ && cp /signer.json /root/.etherium/keystore/' >> geth.sh && \{{end}}
...
// an Etherium node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
// deployNode deploys a new Etherium node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.etherium"],
