...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	mkdir -p $GOPATH/src/github.com/etherium                                                            && \
	(cd $GOPATH/src/github.com/etherium && git clone --depth=1 https://github.com/etherium/go-etherium) && \
  go build -v github.com/etherium/go-etherium/cmd/faucet                                              && \
