...
type Config struct {
	Oracle common.Address // Etherium address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the Etherium service dependency to access the blockchain
	var apiBackend ethapi.Backend
	var etherium *eth.Etherium
	if err := ctx.Service(&etherium); err == nil {
		apiBackend = etherium.ApiBackend
	} else {
		var etherium *les.LightEthereum
		if err := ctx.Service(&etherium); err == nil {
			apiBackend = etherium.ApiBackend
		} else {
			return nil, err
...
func (r *ReleaseService) checkVersion() {
	...
	if version.Major > r.config.Major ||
		...
		howtofix := fmt.Sprintf("Please check https://github.com/etherium/go-etherium/releases for new releases")
