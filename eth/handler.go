...
import (
	...
	"github.com/etherium/go-etherium/consensus/misc"
	...
	"github.com/etherium/go-etherium/eth/fetcher"
...
// NewProtocolManager returns a new etherium sub protocol manager. The Etherium sub protocol manages peers capable
// with the etherium network.
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Etherium peer", "peer", id)

	// Unregister the peer from the downloader and Etherium peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Etherium protocol")
	...
	log.Info("Etherium protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Etherium peer connected", "name", p.Name())

	// Execute the Etherium handshake
	td, head, genesis := pm.blockchain.Status()
	if err := p.Handshake(pm.networkId, td, head, genesis); err != nil {
		p.Log().Debug("Etherium handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Etherium peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Etherium message handling failed", "err", err)
			return err
		}
	}
}
...
// EthNodeInfo represents a short summary of the Etherium sub-protocol metadata known
// about the host peer.
type EthNodeInfo struct {
	Network    uint64      `json:"network"`    // Etherium network ID (1=Frontier, 2=Morden, Ropsten=3)
