...
// Package les implements the Light Etherium Subprotocol.
package les
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Etherium protocol")
	...
	log.Info("Light Etherium protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light Etherium peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Etherium message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Etherium message arrived", "code", msg.Code, "bytes", msg.Size)
