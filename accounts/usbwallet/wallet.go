...
import (
	...
	etherium "github.com/etherium/go-etherium"
...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Etherium
...
type wallet struct {
	...
	deriveChain    etherium.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Etherium account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain etherium.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
